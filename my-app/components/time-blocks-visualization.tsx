...
export default function TimeBlocksVisualization({ todos, period = 'day' }: TimeBlocksVisualizationProps) {
  ...
  const { timeBlocks, gridConfig } = useMemo(() => {
    ...
    const blocks: Array<{ color: string; tagName: string }> = [];
  ...
  // Get title and subtitle based on period
  const getTitle = () => {
    if (period === 'week') return 'Time Blocks of the Week';
    if (period === 'month') return 'Time Blocks of the Month';
    return 'Time Blocks of the Day';
  };

  const getSubtitle = () => {
    if (period === 'week' || period === 'month') {
      return 'Each time block represents one hour';
    }
    return 'Each time block represents 10 minutes';
  };

  const getProportionTitle = () => {
    if (period === 'week') return 'Tag Proportion of the Week';
    if (period === 'month') return 'Tag Proportion of the Month';
    return 'Tag Proportion of the Day';
  };

  const getProportionSubtitle = () => {
    if (period === 'week') {
      return 'Shows the distribution of different tags throughout the week.';
    }
    if (period === 'month') {
      return 'Shows the distribution of different tags throughout the month.';
    }
    return 'Shows the distribution of different tags throughout the day.';
  };
  ...
  const renderTimeBlocksGrid = () => {
    if (period === 'day') {
      ...
      // Hour labels: 0-, 6-, 12-, 18- for better readability
      const hourLabels = [0, 6, 12, 18].map(h => h.toString() + '-');
      ...
      <View style={[styles.overviewSection, { backgroundColor: colors.surface }]}>
        ...
        <View style={styles.overviewMetrics}>
          <View style={styles.metricItem}>
            ...
            <Text style={[styles.metricLabel, { color: colors.textSecondary }]}>Schedule</Text>
          </View>
          <View style={styles.metricItem}>
            <Text style={[styles.metricValue, { color: colors.text }]}>
              {totalHours}h {remainingMinutes}m
            </Text>
            <Text style={[styles.metricLabel, { color: colors.textSecondary }]}>Total Duration</Text>
          </View>
        </View>
      </View>
